...
class GedcomBrowser:
    ...
    def get_individuals(self) -> List[Dict[str, str]]:
        ...
        for indi in self.parser.get_all_individuals():
            ...
            individuals.append(
                {"id": indi.xref_id, "name": name, "birth": birth, "death": death}
            )
            ...
            if child.tag == "FAMS":  # Family where individual is spouse
                ...
                if fam_record:
                    ...
                    for fam_child in fam_record.children:
                        if (
                            fam_child.tag in ["HUSB", "WIFE"]
                            and fam_child.value != indi.xref_id
                        ):
                                ...
                                parents.append(
                                    {
                                        ...
                                        "relation": (
                                            "Father"
                                            if fam_child.tag == "HUSB"
                                            else "Mother"
                                        ),
                                    }
                                )

                    families["parent"].append({"id": fam_id, "parents": parents})
    ...
    try:
        ...
        if individuals:
            ...
            for i, individual in enumerate(individuals[:5]):
                print(
                    f"{i+1}. {individual['name']} ({individual['birth']} - {individual['death']})"
                )
