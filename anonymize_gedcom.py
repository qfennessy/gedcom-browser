    ...
    def load_mappings(self, mapping_file):
        ...
        try:
            with open(mapping_file, 'rb') as f:
                ...
                logger.info(f"Loaded {len(self.given_name_map)} given names, {len(self.surname_map)} surnames, and {len(self.place_map)} places from {mapping_file}")
...
def anonymize_gedcom_file(input_file, output_file=None, overwrite=False, anonymizer=None, save_mappings=True):
    """
    ...
    for i, line in enumerate(content):
        ...
        gender = individual_genders.get(current_individual) if current_individual else None
        ...
        if givn_match and current_individual and 'given' in individual_names[current_individual]:
        ...
        if surn_match and current_individual and 'surname' in individual_names[current_individual]:
    ...
    logger.info(f"Anonymized {name_count} names, {place_count} places, and {other_count} other personal items")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Anonymize personal data in GEDCOM files using realistic fake data")
    ...
    parser.add_argument('--mapping-file', 
                        help='File to store name mappings for consistency across runs')
