...
class GedcomParser:
    ...
    def parse_file(self, file_path: str) -> None:
        ...
        if encoding_from_bom:
            python_encoding = 'utf-8' if encoding_from_bom == Encoding.UTF8 else 'utf-16'
                ...
                for enc in ['utf-8', 'utf-16', 'latin-1', 'ascii']:
                    try:
                        ...
                        for line in lines:
                            ...
                            elif in_gedc_section and level == 2 and tag == 'VERS' and value:
        ...
        if self.version in self.STRICT_BOM_VERSIONS and not bom and self.strict_mode:
            raise GedcomError(f"No valid BOM found. GEDCOM {self.version.value} requires a BOM for UTF-8 or UTF-16")
            ...
            for try_encoding in [open_encoding, 'utf-8', 'latin-1']:
                try:
                    ...
                    if try_encoding == 'latin-1':  # Last resort failed
                        raise GedcomError("Unable to decode file with any supported encoding")
                    continue  # Try next encoding
    
    def _detect_encoding_from_bom(self, file_path: str) -> Tuple[Optional[Encoding], bytes]:
        """
    ...
    def _parse_gedcom(self, file: TextIO) -> None:
        ...
        for line_num, line in enumerate(file, 1):
            ...
            if not match:
                if self.strict_mode:
                    raise GedcomError(f"Invalid GEDCOM format at line {line_num}: {line}")
                else:
            ...
            if level > current_level + 1 and self.strict_mode:
                raise GedcomError(f"Skipped level at line {line_num} (jumped from {current_level} to {level})")
            ...
            if tag in ('CONT', 'CONC'):
                if not self._current_record_stack:
                    if self.strict_mode:
                        raise GedcomError(f"{tag} tag without parent record at line {line_num}")
                    else:
                ...
                if (self.version == Version.V555 and self.strict_mode and 
                    header_record and self._is_under_header(parent)):
                    raise GedcomError(f"CONC or CONT tags are not allowed in the basic header at line {line_num}")
            ...
            if level == 0:
                ...
                if xref_id:
                    if xref_id in self.records and self.strict_mode:
                        raise GedcomError(f"Duplicate XREF ID: {xref_id} at line {line_num}")
                    self.records[xref_id] = record
                
                # Process header
                if tag == 'HEAD':
                    if header_found and self.strict_mode:
                        raise GedcomError(f"Multiple HEAD records found at line {line_num}")
                    header_found = True
    ...
    def _is_under_header(self, record: GedcomRecord) -> bool:
        ...
        stack_idx = self._current_record_stack.index(record) if record in self._current_record_stack else -1
        if stack_idx < 0:
            return False
            
        for i in range(stack_idx, -1, -1):
            if self._current_record_stack[i].level == 0 and self._current_record_stack[i].tag == 'HEAD':
                return True
    ...
    def _validate_header(self, header_record: GedcomRecord) -> None:
        ...
        if self.strict_mode and detected_version != Version.V555:
            raise GedcomError(f"Unsupported GEDCOM version: {vers_records[0].value}. Only 5.5.5 is supported in strict mode")
        ...
        if self.version in [Version.V551, Version.V555]:
            ...
            if form_records[0].value != 'LINEAGE-LINKED':
                raise GedcomError(f"Unsupported GEDCOM form: {form_records[0].value}. Only LINEAGE-LINKED is supported")
        ...
        if char_records:
            ...
            try:
                ...
                if self.version == Version.V555 and self.strict_mode:
                    if file_encoding not in [Encoding.UTF8, Encoding.UTF16, Encoding.UNICODE]:
                        raise GedcomError(f"Invalid encoding for GEDCOM 5.5.5: {file_encoding.value}. "
        ...
        for child in header_record.children:
            if child.tag in ('CONC', 'CONT'):
                raise GedcomError("CONC or CONT tags are not allowed in the basic header")
