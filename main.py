...
def validate_file(file_path: str, relaxed_mode: bool = False) -> bool:
    ...
    try:
        ...
        if relaxed_mode:
            print(
                f"✓ File '{file_path}' is a valid GEDCOM {version_str} file (relaxed mode)"
            )
        else:
        ...
        if relaxed_mode:
            print(
                f"✗ File '{file_path}' is not a valid GEDCOM file (even in relaxed mode):"
            )
        else:
...
def list_individuals(browser: GedcomBrowser) -> None:
    ...
    for individual in individuals:
        print(
            f"{individual['id']:<12} {individual['name']:<40} "
...
def show_individual(browser: GedcomBrowser, xref_id: str) -> None:
    ...
    if details["events"]:
        ...
        for event in details["events"]:
            print(
                f"{event['type']:<5} Date: {event['date']:<30} "
                f"Place: {event['place']}"
            )
