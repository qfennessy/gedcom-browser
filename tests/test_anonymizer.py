...
def test_anonymizer_initialization():
    ...
    custom_anonymizer = GedcomAnonymizer(seed=100, preserve_structure=False, mapping_file="test.pkl")
...
0 TRLR"""
    ...
    try:
        ...
        assert "2 DATE 1 JAN 1950" in anonymized_content  # Dates should not be anonymized
...
0 TRLR"""
        ...
        assert processed >= 1  # At least the file in the subdirectory should be processed
...
0 TRLR"""
    ...
    try:
        ...
        male_name_match = re.search(r'@I1@ INDI\s+1 NAME\s+([^/]+)/', anonymized_content)
        female_name_match = re.search(r'@I2@ INDI\s+1 NAME\s+([^/]+)/', anonymized_content)
