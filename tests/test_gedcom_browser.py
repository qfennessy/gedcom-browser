...
def test_family_relationships():
    ...
    try:
        ...
        father = next((p for p in parent_family['parents'] if p['relation'] == 'Father'), None)
        mother = next((p for p in parent_family['parents'] if p['relation'] == 'Mother'), None)
...
0 TRLR"""
    ...
    try:
        ...
        def _mock_validate(rec):
            ...
            if vers.value == '5.5.1':
                raise GedcomError("Unsupported GEDCOM version: 5.5.1. Only 5.5.5 is supported")
